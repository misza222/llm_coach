...
- `persistence/` — `PersistenceBackend` Protocol with `InMemoryBackend` and `SqlBackend` (SQLite/PostgreSQL) implementations. `tables.py` defines shared SQLAlchemy table definitions (sessions, users, oauth_accounts, anonymous_message_counts).
